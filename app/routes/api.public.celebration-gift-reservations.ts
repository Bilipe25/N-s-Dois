...
export async function action({ request }: Route.ActionArgs) {
  ...
  const { data: gift } = await supabase.from("bridal_shower_gifts").select("id").eq("id", parsed.data.giftId).maybeSingle();
  if (!gift) return Response.json({ error: "Presente indisponível." }, { status: 404, headers: noStoreHeaders() });
