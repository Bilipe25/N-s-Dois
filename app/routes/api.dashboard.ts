...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const people = (onlyConfirmed: boolean) => guestRows
    .filter((guest) => !onlyConfirmed || guest.rsvp_status === "confirmado")
    .reduce((sum, guest) => sum + (guest.adults_count ?? 0) + (guest.children_count ?? 0), 0);
  const budgetRows = budget.data ?? [];

  return Response.json({
    config: config.data,
    tasks: { pending: pendingTasks.count ?? 0, total: totalTasks.count ?? 0 },
    guests: { confirmed: people(true), total: people(false) },
    budget: {
